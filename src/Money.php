...
final readonly class Money extends AbstractMoney implements Stringable
{
     ...
     * @return self                   The smallest of the given monies.
     */
     ...
     *
     * @throws MoneyMismatchException If all the monies are not in the same currency.
     * @return self                   The largest of the given monies.
     */
     ...
     * @return self                   The sum of all the given monies.
     */
    ...
    /**
     * Returns the exact ratio of this Money to the given Money as a dimensionless BigRational.
     *
     * @throws DivisionByZeroException If the given Money amount is zero.
     * @throws MoneyMismatchException  If the currencies do not match.
     */
    public function ratioOf(self $that): BigRational
    {
        return $this->amount->toBigRational()->dividedBy(
            $this->getAmountOf($that),
        );
    }

    /**
     * Returns the percentage that this Money represents of the given Money.
     *
     * This is equivalent to ratioOf() multiplied by 100.
     *
     * @throws DivisionByZeroException If the given Money amount is zero.
     * @throws MoneyMismatchException  If the currencies do not match.
     */
    public function percentageOf(self $that): BigRational
    {
        return $this->ratioOf($that)->multipliedBy(100);
    }

    /**
     * Returns a TaxResult treating this Money as the net amount (before tax).
     *
     * Computes gross = net * (1 + rate/100), with tax derived as gross - net
     * to preserve the invariant net + tax = gross after rounding.
     *
     * @param TaxRate      $rate         The tax rate to apply.
     * @param RoundingMode $roundingMode The rounding mode for the gross amount.
     *
     * @throws RoundingNecessaryException If RoundingMode::Unnecessary is passed and rounding is necessary.
     * @return TaxResult                  The tax result with net, gross, and tax amounts.
     */
    public function includingTax(TaxRate $rate, RoundingMode $roundingMode = RoundingMode::HalfUp): TaxResult
    {
        if ($rate->isZero()) {
            $zero = self::zero($this->currency, $this->context);

            return TaxResult::create($this, $this, $zero, $rate);
        }

        $grossAmount = $this->amount->toBigRational()->multipliedBy(
            $rate->getMultiplier()->plus(1),
        );

        $gross = self::create($grossAmount, $this->currency, $this->context, $roundingMode);
        $tax = $gross->minus($this);

        return TaxResult::create($this, $gross, $tax, $rate);
    }

    /**
     * Returns a TaxResult treating this Money as the gross amount (after tax).
     *
     * Computes net = gross / (1 + rate/100), with tax derived as gross - net
     * to preserve the invariant net + tax = gross after rounding.
     *
     * @param TaxRate      $rate         The tax rate to apply.
     * @param RoundingMode $roundingMode The rounding mode for the net amount.
     *
     * @throws RoundingNecessaryException If RoundingMode::Unnecessary is passed and rounding is necessary.
     * @return TaxResult                  The tax result with net, gross, and tax amounts.
     */
    public function excludingTax(TaxRate $rate, RoundingMode $roundingMode = RoundingMode::HalfUp): TaxResult
    {
        if ($rate->isZero()) {
            $zero = self::zero($this->currency, $this->context);
        ...
        $netAmount = $this->amount->toBigRational()->dividedBy(
            $rate->getMultiplier()->plus(1),
        );

        $net = self::create($netAmount, $this->currency, $this->context, $roundingMode);
        $tax = $this->minus($net);

        return TaxResult::create($net, $this, $tax, $rate);
    }

    /**
     * Returns the tax amount for this Money at the given rate.
     *
     * This is a convenience method equivalent to $this->includingTax($rate)->getTax().
     *
     * @param TaxRate      $rate         The tax rate to apply.
     * @param RoundingMode $roundingMode The rounding mode for the calculation.
     *
     * @throws RoundingNecessaryException If RoundingMode::Unnecessary is passed and rounding is necessary.
     */
    public function taxAmount(TaxRate $rate, RoundingMode $roundingMode = RoundingMode::HalfUp): self
    {
        return $this->includingTax($rate, $roundingMode)->getTax();
    }

    /**
     * Returns a DiscountResult treating this Money as the original amount (before discount).
     *
     * Computes discounted = original * (1 - rate/100), with savings derived as
     * original - discounted to preserve the invariant original - savings = discounted
     * after rounding.
     *
     * @param DiscountRate $rate         The discount rate to apply.
     * @param RoundingMode $roundingMode The rounding mode for the discounted amount.
     *
     * @throws RoundingNecessaryException If RoundingMode::Unnecessary is passed and rounding is necessary.
     * @return DiscountResult             The discount result with original, discounted, and savings amounts.
     */
    public function applyDiscount(DiscountRate $rate, RoundingMode $roundingMode = RoundingMode::HalfUp): DiscountResult
    {
        if ($rate->isZero()) {
            $zero = self::zero($this->currency, $this->context);

            return DiscountResult::create($this, $this, $zero, $rate);
        }

        $discountedAmount = $this->amount->toBigRational()->multipliedBy(
            BigRational::of(1)->minus($rate->getMultiplier()),
        );

        $discounted = self::create($discountedAmount, $this->currency, $this->context, $roundingMode);
        $savings = $this->minus($discounted);

        return DiscountResult::create($this, $discounted, $savings, $rate);
    }

    /**
     * Returns the savings amount for this Money at the given discount rate.
     *
     * This is a convenience method equivalent to $this->applyDiscount($rate)->getSavings().
     *
     * @param DiscountRate $rate         The discount rate to apply.
     * @param RoundingMode $roundingMode The rounding mode for the calculation.
     *
     * @throws RoundingNecessaryException If RoundingMode::Unnecessary is passed and rounding is necessary.
     */
    public function discountAmount(DiscountRate $rate, RoundingMode $roundingMode = RoundingMode::HalfUp): self
    {
        return $this->applyDiscount($rate, $roundingMode)->getSavings();
    }

    /**
     * Formats this Money to the given locale.
     *
     * Note: this method uses NumberFormatter internally, which converts amounts to floating-point,
     ...
     *
     * @return string The locale-formatted monetary value (e.g. "$1,234.56" for en_US).
     */
     ...
     *
     * @return string The exact formatted value (e.g. "USD 1234.56").
     */
